...
| **A4** | Creating and deleting rules — **next** | Create a rule for an item Alex owns; delete a custom one; restore defaults. **Documented precedence** (explicit trip decision → user rule → accepted learned preference → system default), which must be written before user-created rules exist, because that is the moment two rules can target one item. |
