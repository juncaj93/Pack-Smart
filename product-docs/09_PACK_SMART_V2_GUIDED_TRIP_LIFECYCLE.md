...
| Packing rules | complete (A4b) | Creatable, editable, disable/restore, deletable where safe |
| Rule precedence | complete (A4a, A4b) | `technical-docs/11_RULE_PRECEDENCE.md`, written from the engine and pinned by tests |
...
| **A4a** | Precedence — **shipped** | `technical-docs/11_RULE_PRECEDENCE.md`, written from the engine rather than from intent, and pinned by `rule-precedence.test.ts`. It found a real gap: `fixed_per_trip` **assigned** the base where every other quantity rule takes a maximum, so "always pack 3" plus "2 per day" gave 3 or 20 depending on row order. Nothing seeded pairs them; a user rule can, on day one. Settled and corrected in A4b. |
| **A4b** | Creating and deleting rules — **shipped** | Both decisions were ruled on and are implemented. `packing_rule` gained `source` (`system`/`user`/`learned`) **and** `supersedes_rule_id`, because a source column alone cannot say *which* default a rule replaces without guessing from row order or names. A default is now never written to: editing one stores a user-owned copy that supersedes it, `Use the default` removes the copy, and switching one off is a copy that contributes nothing. §1.1's gap is settled — `fixed_per_trip` is a floor, so *always pack 3* plus *2 per day* is 20 on a ten-day trip whichever row comes first. Create four kinds of rule, delete the ones Alex wrote. |
